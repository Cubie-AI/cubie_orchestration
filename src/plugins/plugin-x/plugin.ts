...
import { z } from "zod";
import { generateTweetTemplate } from "./templates.js";
import { XPlatformContext, XPluginConfig } from "./types.js";
    ...
    while (retries > 0) {
      try {
        ...
        if (await this.scraper.isLoggedIn()) {
          ...
          this.profile = await this.scraper.getProfile(this.config.username);
          return;
        }
    ...
    try {
      ...
      try {
        for await (const tweet of tweetsGenerator) {
          log.info(JSON.stringify(tweet, null, 2));
          if (
            !tweet.id ||
            !tweet.username ||
            tweet.userId === this.profile?.userId
          ) {
            log.warn(
              `Invalid tweet received: ${JSON.stringify(tweet, null, 2)}`
            );
            continue;
          }
